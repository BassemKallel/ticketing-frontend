...
const NotificationsPage = () => {
    ...
    // Ajout: Recharger les notifications au montage et écouter les nouvelles
    useEffect(() => {
        fetchNotifications();
        
        // Cette partie dépend de votre implémentation WebSocket
        // Si vous utilisez Echo ou autre, adaptez cette partie
        const channel = window.Echo?.private(`App.Models.User.${user.id}`);
        
        if (channel) {
            channel.listen('.notification.created', (data) => {
                fetchNotifications(); // Recharge les notifications
                                ...
                                <ul className="space-y-2">
                                    {groupedNotifications[groupName].map(notif => (
                                        ...
                                        >
                                            ...
                                            {!notif.read_at && (
                                                <div 
                                                    className="h-2.5 w-2.5 rounded-full bg-orange-500 flex-shrink-0 self-center" 
                                                    title="Non lue"
                                                />
                                            )}
                                        </li>
